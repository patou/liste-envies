    ...
    function n(n, r) {
      ...
      function S(e) {
        if (
          ...
          p.time - C[C.length - 1].time)
        ) {
          for (var t = 0; t < C.length - 1 && p.time - C[t].time > 80; t++);
          (m = {
            x: (p.x - C[t].x) / (p.time - C[t].time),
            y: (p.y - C[t].y) / (p.time - C[t].time)
          }),
            C.length >= 5 && C.shift(),
            C.push({ x: p.x, y: p.y, time: p.time });
        }
      }
      function c(e, n) {
        o ||
      ...
      function u(e) {
        ("x" == P.axis && ((!P.preventScroll && A) || g[h](e))) ||
          I(e.target) ||
          (S(e),
          (Math.abs(p.x) > P.clickTolerance ||
            Math.abs(p.y) > P.clickTolerance) &&
            (B = !1),
          (A === t &&
            3 !== h &&
            (A = Math.abs(p.x) < Math.abs(p.y) && !P.preventScroll)) ||
            (P.move(e, f, p, m, G) && P.preventDefault && l(e)));
      }
      function d(e) {
        h && S(e),
          !B && e.target && e.target.blur && e.target.blur(),
          0 !== h && T(document, v[h][1], u, !1),
      ...
      function b() {
        H.push(
          s(document, "scroll", function(e) {
            (window.nextendScrollFocus !== t && window.nextendScrollFocus) ||
              (G = !0);
          })
        ),
          H.push(
      ...
      if ("object" == typeof e) for (var n in e) E(e, n) && s(n, e[n]);
        ...
        (t = "function" == typeof t ? t() : t),
          1 == i.length
            ? (p[i[0]] = t)
            : (!p[i[0]] ||
                p[i[0]] instanceof Boolean ||
                (p[i[0]] = new Boolean(p[i[0]])),
              (p[i[0]][i[1]] = t)),
          a([(t && 0 != t ? "" : "no-") + i.join("-")]),
          p._trigger(e, t);
      }
