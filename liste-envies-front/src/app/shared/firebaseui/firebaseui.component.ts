...
export class FirebaseUIComponent implements OnInit, OnDestroy {
  ...
  signInSuccessCallback: EventEmitter<
    FirebaseUISignInSuccess
  > = new EventEmitter();
  ...
  private getUIAuthConfig(authConfig: FirebaseUIAuthConfig): Object {
    ...
    for (let provider of authConfig.providers) {
      if (!!(provider as AuthProviderWithCustomConfig).customConfig) {
        ...
        providerWithConfig["provider"] = FirebaseUIComponent.getAuthProvider(
          provider.provider
        );
        ...
        authProviders.push(
          FirebaseUIComponent.getAuthProvider(provider as AuthProvider)
        );
      }
    }
  ...
  private firebaseUIPopup() {
    ...
    firebaseUiInstance.start(
      ...
      this.getUIAuthConfig(this.firebaseUiConfig)
    );
  }
}
